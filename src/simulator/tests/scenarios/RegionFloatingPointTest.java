 ...
 * Covers: Contains (Strict), Intersects, Expand, Intersection, Union, and Static Optimization Parity.
 */
public class RegionFloatingPointTest extends RegionTestScenario {
    ...
    @Override
    public String getTestName() { return "Region Floating-Point Logic (Verbose + Union)"; }
    ...
    private boolean testStaticVsInstanceEquivalence() {
        ...
        // Check Intersection Area Parity
        double instanceVal = rStd1.getIntersectionArea(rStd2);
        float staticVal = Region.fastIntersectionArea(10f, 20f, 10f, 20f, 15f, 25f, 15f, 25f);
        
        logger.info(String.format("   Parity Check (Inter Area): Instance=%.4f vs Static=%.4f", instanceVal, staticVal));
        if (Math.abs(instanceVal - staticVal) > TOLERANCE) {
            logger.severe("   [FAIL] Mismatch in Intersection Optimization Logic!");
            pass = false;
        } else {
            logger.info("   [PASS] Intersection Parity");
        }

        // MBR Equivalence
        pass &= checkMBREquivalence("MBR Std", rStd1, rStd2);
        pass &= checkMBREquivalence("MBR Wrap", rWrap, rInGap);
        pass &= checkMBREquivalence("MBR Bridge", rLeft, rRight);
    ...
    private boolean checkMBREquivalence(String label, Region r1, Region r2) {
        ...
        logger.info(String.format("   [%s] %s (MBR Area): Instance=%.4f, Static=%.4f", status, label, expectedArea, calculatedArea));
