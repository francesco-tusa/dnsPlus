...
public class MarketplaceRegionStore extends AbstractMultiRegionStore {
    ...
    private static class RegionQuadTree {
        ...
        private RegionQuadTree(double minLon, double minLat, double maxLon, double maxLat, int depth) {
            ...
            this.items = new ArrayList<>(); this.depth = depth;
        }
        public void insert(SubscriptionWithRegion sub) {
            if (cachedMBR == null) cachedMBR = new Region(sub.getRegion()); else cachedMBR.expand(sub.getRegion());
            if (children != null) {
                int index = getIndex(sub.getRegion());
                if (index != -1) { children[index].insert(sub); return; }
            }
            items.add(sub);
            if (items.size() > MAX_ITEMS && depth < MAX_DEPTH) {
                ...
                while (it.hasNext()) {
                    ...
                    if (index != -1) { children[index].insert(s); it.remove(); }
                }
            }
        }
        public boolean remove(SubscriptionWithRegion sub) {
            ...
            if (!removed && children != null) {
                ...
                else for (RegionQuadTree child : children) if (child.remove(sub)) { removed = true; break; }
            }
        ...
        private void recalculateMBR() {
            ...
            for (SubscriptionWithRegion s : items) {
                if (cachedMBR == null) cachedMBR = new Region(s.getRegion()); else cachedMBR.expand(s.getRegion());
            }
            if (children != null) for (RegionQuadTree c : children) if (c.getMBR() != null) {
                if (cachedMBR == null) cachedMBR = new Region(c.getMBR()); else cachedMBR.expand(c.getMBR());
            }
        }
        ...
        public boolean containsPoint(Location loc, int[] ops) {
            ops[0]++; if (cachedMBR == null || !cachedMBR.contains(loc)) return false;
            for (SubscriptionWithRegion s : items) { ops[0]++; if (s.getRegion().contains(loc)) return true; }
            if (children != null) {
        ...
        public List<SubscriptionWithRegion> findCandidatesContaining(Region query) {
            ...
            for (SubscriptionWithRegion s : items) if (s.getRegion().contains(query)) c.add(s);
            if (children != null) {
        ...
        public List<SubscriptionWithRegion> findIntersections(Region query) {
            ...
            for (SubscriptionWithRegion s : items) if (s.getRegion().intersects(query)) h.add(s);
            if (children != null) for (RegionQuadTree child : children) {
                if (child.getMBR() != null && child.getMBR().intersects(query)) h.addAll(child.findIntersections(query));
            }
            return h;
        }
        private void split() {
